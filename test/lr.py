...
# 随机梯度上升法+逻辑回归
def stocGradAscent_logRes(dataMatrix, classLabels, test_mat, test_label, maxIter, alpha):
    ...
    for i in xrange(TEST):
        res = sigmoid(sum(test_mat[i] * weights))
        # print res
        if res > 0.5:
...
def main():
    ...
    alpha = 0.01
    maxIter = 500
    stocGradAscent_logRes(train_mat, train_label, test_mat, test_label, maxIter, alpha)
    # f = open('out.txt', 'w')
    # for i in w:
    #     print >> f, i
